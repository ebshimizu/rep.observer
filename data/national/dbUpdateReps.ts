...
console.log(`fetching data for congress ${congress}}`)
...
async function processRep(
  bioguideId: string,
  sessions: { house: number; senate: number }
) {
  ...
  if (bioData) {
    ...
    const mostRecentTermData =
      fullData != null ? fullData.terms[fullData.terms.length - 1] : null
    ...
    const updateTerm = await supabase.from('terms').upsert({
      ...
      district:
        mostRecentTerm.chamber === 'Senate'
          ? null
          : mostRecentTerm?.district ?? 1,
      party,
    })
...
async function processReps(sessions: { house: number; senate: number }) {
  ...
  const repData = JSON.parse(fs.readFileSync(repFile).toString()) as Record<
    string,
    RepCacheData
  >
...
async function createTermData(): Promise<{ house: number; senate: number }> {
  ...
  if (house.data && house.data.length > 0) {
    ...
    const houseInsert = await supabase
      .from('sessions')
      .insert({
        level: 'national',
        chamber: 'House of Representatives',
    ...
    if (houseInsert.data) {
      houseId = houseInsert.data.id
    } else {
      console.error('No ID returned from insert operation')
    }
  }
  ...
  if (senate.data && senate.data.length > 0) {
    ...
    const senateInsert = await supabase
      .from('sessions')
      .insert({
        level: 'national',
        chamber: 'Senate',
...
if (sessions.house == null || sessions.senate == null) {
  console.error('Session IDs not found, cannot proceed with member update')
} else {
